...
- If the research prepass produced a raw `report_artifact`, each initial planner step must return `proposals.sprint_plan_update.research_report_read` proving the full raw Markdown report was read for that phase. Missing or wrong receipts keep the phase invalid.
- Researcher-synthesized `RG-*` todo coverage requirements are stored in `sprint_state["research_prepass"]`; `backlog_definition` and `todo_finalization` must return a `research_coverage_matrix` and collectively cover every `RG-*` item.
...
- Sprint backlog definition items should carry concrete `acceptance_criteria` plus planner trace in `origin.milestone_ref`, `origin.requirement_refs`, `origin.spec_refs`, and `origin.research_refs` when a source-backed or local-evidence research report is available. When research synthesis produces `RG-*` todo coverage requirements, covering backlog items also carry `origin.research_coverage_refs`, and selected TODOs carry `research_coverage_refs` plus matching `research_refs`.
