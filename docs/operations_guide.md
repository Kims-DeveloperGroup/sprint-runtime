...
- After initial `artifact_sync`, planner must return `proposals.initial_implementation_plan` instead of defining backlog/TODOs. Orchestrator posts that implementation plan to the configured relay channel with the actual requester mentioned and mirrors a progress report to the configured Discord report channel.
