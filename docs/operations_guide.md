...
- The research prepass includes `requirement_traceability_matrix` rows for every closeout-required `REQ-*`. Missing local evidence reopens research for those rows; if Deep Research fails during sprint prepass, the handoff is still `completed` with failed RTM rows, empty `backing_sources`, and unresolved risk guidance for planner.
- After initial `artifact_sync`, planner must return Codex-plan-mode style `proposals.initial_implementation_plan` instead of defining backlog/TODOs. The plan includes canonical sections plus `PLAN-ACT-*` action records built from kickoff requirements, original `REQ-*`, reference artifacts, `spec.md`, and research report/prepass evidence.
- Orchestrator normalizes and stores the plan in `initial_plan_confirmation.draft_proposal`, records `initial_plan_confirmation.plan_artifact`, renders sprint `plan.md` from the same content, posts that implementation plan to the configured relay channel with the actual requester mentioned, and mirrors a progress report to the configured Discord report channel.
...
- Planner may define or reopen sprint-relevant backlog from the current milestone, kickoff requirements, research findings, `spec.md`, and confirmed implementation plan only after the plan is confirmed.
- Backlog items and TODOs execute confirmed `PLAN-ACT-*` actions. Non-supporting items must carry matching `plan_action_refs`; startup validation blocks if a non-supporting action lacks persisted backlog or TODO coverage.
