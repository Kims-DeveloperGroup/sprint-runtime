...
- an internal non-public sourcer agent can advance one active CLI-created goal by deriving a stop condition, evaluating linked sprint history plus bounded shared-workspace Markdown context, and proposing one sprint-ready next milestone when no sprint is active; the milestone must include kickoff requirements and a completion condition, that completion condition is carried as a kickoff requirement, and the sourcer does not create backlog records or planner review requests
...
- current implemented pure-policy boundary:
  ...
  - `workflows/orchestration/notifications.py` owns startup report rendering, boxed-report excerpt summarization, low-level Discord chunking, runtime signature tagging, cross-process send locking, startup fallback recovery, requester-status message formatting, requester reply delivery, immediate receipts, sprint completion user-summary delivery, sprint progress report delivery, internal relay summary delivery, and Discord relay-envelope sending
  ...
  - `runtime/internal/goal_sourcing.py` owns the internal sourcer runtime, goal-sourcing payload normalization, and sourcing-monitoring receipts
...
- current implemented module split:
  ...
  - `workflows/orchestration/notifications.py` owns low-level Discord notification delivery, requester summary simplification, requester status-message assembly, requester reply-route recovery / dispatch glue, channel reply delegation, immediate-receipt trusted-relay suppression, generic Discord content send delegation, and startup notification send/fallback state glue
