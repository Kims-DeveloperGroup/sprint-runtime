...
- `teams_runtime/workflows/orchestration/team_service.py`
  - canonical home of `TeamService` and orchestration-side composition, with manual sprint ingress parsing, implementation-plan feedback, and requirement-candidate intake delegated through `workflows/orchestration/ingress.py`, delegated request/result handling delegated through `workflows/orchestration/delegation.py`, relay transport/consume glue delegated through `workflows/orchestration/relay.py`, requester-facing status/reply/channel/receipt/startup send shaping delegated through `workflows/orchestration/notifications.py`, and sprint lifecycle execution delegated through `workflows/sprints/lifecycle.py`; `core/orchestration.py` remains compatibility-only
...
- during an active sprint, high-confidence user requirement additions are captured as `REQ-CAND-*` records in `pending_requirement_candidates` and do not create request/backlog/TODO scope by themselves
- planner receives pending candidates only during an `ongoing_review` checkpoint after a TODO reaches `completed` or `committed`, then may return `proposals.sprint_requirement_reconciliation` to register accepted scope as new `REQ-*`, merge it with an existing `REQ-*`, defer it, or reject it
- registered mid-sprint `REQ-*` requirements may modify only undone requirements, undone/new backlog items, specs, and newly created TODOs; completed or committed TODOs are fixed evidence
