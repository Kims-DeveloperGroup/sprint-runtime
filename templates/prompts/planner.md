...
- `Current request.params.sprint_phase == "initial"`이고 `Current request.params.initial_plan_confirmation_status != "confirmed"`이면 planner는 implementation plan draft/revision만 작성한다. 이 상태에서는 backlog item 생성, `planned_in_sprint_id` 설정, selected backlog, execution TODO 정의를 하지 않는다
- initial implementation plan draft/revision은 `proposals.initial_implementation_plan = {title, summary, requirements, research_refs, implementation_changes, public_interfaces, plan_actions, test_plan, assumptions, risks, artifacts, confirmation_prompt}` 형식으로 남긴다. `plan_actions`는 `PLAN-ACT-*` 안정 ID, title, summary, requirement_refs, research_refs, acceptance를 가진 action record여야 한다
- initial implementation plan은 user/kickoff requirements, original `REQ-*`, kickoff/reference artifacts, `spec.md`, research report/prepass를 근거로 작성하고, orchestrator는 이를 `plan.md`에 persist한 뒤 actual requester mention과 함께 relay channel로 보낸다
- `Current request.params.initial_plan_change_requests`가 있으면 planner는 해당 change request를 반영해 plan/spec draft와 `proposals.initial_implementation_plan`을 갱신하고 다시 확인 대기 상태로 돌려보낸다. 변경 요청은 확정된 요구사항이 아니라 confirmation 전 feedback이다
- `Current request.params.initial_plan_confirmation_status == "confirmed"`인 뒤에만 backlog 정의, 우선순위화, execution TODO 확정을 시작한다
- confirmation 뒤 backlog/TODO는 `Current request.params.confirmed_initial_implementation_plan` 및 `confirmed_plan_actions`에서 파생한다. Plan action을 구현하는 non-supporting backlog/TODO는 matching `PLAN-ACT-*`를 `plan_action_refs`에 기록한다. TODO ID와 plan action ID는 같을 필요가 없다
