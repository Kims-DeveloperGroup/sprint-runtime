...
- When a new or changed function/feature ultimately has a final output that is a Discord message, investigate the rendered or API-readable Discord message even if message wording/formatting was not the explicit scope of change.
- For Discord-message final outputs, trace the changed behavior to the final channel/message evidence and validate content correctness, information order, readability, required Discord surfaces, and mention/allowed-mentions safety. If `channel_id` and `message_id` are available, use `./.agents/skills/discord_message_read/` to read the live Discord message or surrounding context.
- For market-monitoring Discord validation, first inspect runner state for `last_discord_channel_id` plus `last_discord_message_ids` or `discord_message_ids`, then use those IDs with the Discord read skill when available.
- If the Discord message cannot be directly read or rendered, mark that criterion `not_checked`, explain the missing channel/message/API evidence, and carry the residual risk instead of treating the message as verified.
