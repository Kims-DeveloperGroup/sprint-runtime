...
- For pre-confirmation initial planning, return `proposals.initial_implementation_plan = {{"title":"...","summary":"...","requirements":[],"approach":[],"risks":[],"artifacts":[],"confirmation_prompt":"..."}}` so orchestrator can send it to the requester and mirror it to the report channel.
...
- If `Current request.params.sprint_phase == "initial"` and `Current request.params.initial_phase_step` is set, treat the request as a planner-only substep:
  ...
  - `artifact_sync`: update plan/spec/iteration-log artifacts only. Before user confirmation, this step must also return `proposals.initial_implementation_plan` and then stop without backlog/TODO creation.
  - `backlog_definition`: derive sprint-relevant backlog from the current milestone, kickoff requirements, research prepass, and `spec.md`; create or reopen backlog items before any selection. Backlog zero is invalid. Every backlog item must include concrete `acceptance_criteria` plus `origin.milestone_ref`, `origin.requirement_refs`, and `origin.spec_refs`; it must also be linkable to the active sprint through `milestone_title` or `origin.sprint_id`. If source-backed research exists, every sprint-relevant backlog item must also include `origin.research_refs`.
  ...
  - `todo_finalization`: finalize the execution-ready sprint backlog set and persist `planned_in_sprint_id` for chosen items. Verify every `REQ-*` has a requirement-slice todo, and mark any supporting todo with `supporting_todo: true` plus the `REQ-*` it supports.
