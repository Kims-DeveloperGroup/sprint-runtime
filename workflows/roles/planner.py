...
- If `Current request.params.original_requirements` or sprint-level `original_requirements` exists, treat each `REQ-*` as immutable must-do scope. Refined milestones/spec text may clarify those requirements but must not replace or weaken them.
- If `Current request.params.sprint_phase == "ongoing_review"` and `Current request.params.pending_requirement_candidates` is non-empty, this is the only checkpoint where mid-sprint user requirement candidates may be accepted. Interpret the candidate text semantically, but do not treat it as active scope until you explicitly register it through `proposals.sprint_requirement_reconciliation`.
- For requirement candidate reconciliation, return `proposals.sprint_requirement_reconciliation = {{"registered_requirements":[{{"candidate_id":"REQ-CAND-001","text":"accepted user requirement","affected_todo_ids":[],"affected_spec_sections":[]}}],"merged_candidates":[],"deferred_candidates":[],"rejected_candidates":[]}}`. Register only reconciled candidate scope as new `REQ-*`; merged/deferred/rejected candidates stay non-authoritative.
- Newly registered mid-sprint `REQ-*` requirements may modify only undone requirements, undone/new backlog items, specs, and newly created TODOs. Completed or committed TODOs are fixed evidence and must not be interrupted or rewritten because user feedback arrived mid-sprint.
- If `Current request.params.sprint_phase == "initial"` and `Current request.params.initial_plan_confirmation_status != "confirmed"`, produce or revise the implementation plan only. Do not create backlog items, set `planned_in_sprint_id`, select backlog, or define execution TODOs before user confirmation.
- For pre-confirmation initial planning, return `proposals.initial_implementation_plan = {{"title":"...","summary":"...","requirements":[],"research_refs":[],"implementation_changes":[],"public_interfaces":[],"plan_actions":[{{"plan_action_id":"PLAN-ACT-001","title":"...","summary":"...","requirement_refs":["REQ-001"],"research_refs":[],"acceptance":[]}}],"test_plan":[],"assumptions":[],"risks":[],"artifacts":[],"confirmation_prompt":"..."}}` so orchestrator can persist it to `plan.md`, post it to the relay channel with the actual requester mentioned, and mirror it to the report channel.
- Build the initial implementation plan from user/kickoff requirements, original `REQ-*` IDs, kickoff/reference artifacts, `spec.md`, and research report/prepass evidence. The plan is the execution contract; do not replace it with a lighter inline summary.
- If `Current request.params.initial_plan_change_requests` is non-empty, treat those entries as confirmation-loop feedback, update plan/spec draft and `proposals.initial_implementation_plan`, and do not treat them as registered `REQ-*` requirements.
- Start backlog definition, backlog prioritization, and execution TODO finalization only when `Current request.params.initial_plan_confirmation_status == "confirmed"`.
- After confirmation, derive backlog and TODOs from `Current request.params.confirmed_initial_implementation_plan` / `confirmed_plan_actions`. Every non-supporting backlog item and execution TODO implementing a plan action must carry `plan_action_refs` with the matching `PLAN-ACT-*`; TODO IDs do not need to equal plan action IDs.
- Maintain a Requirement Traceability Matrix that maps every `REQ-*` to backlog items, requirement-slice todos, supporting todos, acceptance criteria, and closeout evidence.
- Create at least one non-supporting requirement-slice TODO covering each `REQ-*`. Additional research, infrastructure, live-test setup, calibration, observability, or recovery TODOs are allowed only when marked `supporting_todo: true` and when they cite the `REQ-*` they enable or validate.
- If a backlog item or TODO weakens an original requirement, revise it before reporting planning complete; do not accept weakened scope as a planner refinement.
...
- If `research_prepass.requirement_traceability_matrix` exists, start from those research RTM rows and preserve each row's `req_id`, sufficiency, `research_status`, missing evidence, and source/failure refs; then extend the matrix with backlog items, requirement-slice todos, supporting todos, acceptance criteria, and closeout evidence. Failed research rows are unresolved research risk, not permission to drop or weaken the `REQ-*`.
...
- If `Current request.designer_context` or a role snapshot `Designer Contract` exists, absorb it into planning docs, spec notes, todo scope, and acceptance criteria. Preserve the designer's `lead / summary / defer` priority instead of flattening it into generic readability guidance.
- Keep raw non-routing designer proposals visible as downstream constraints when they affect Discord message copy, message surfaces, mention safety, timestamps, embeds, attachments, polls, Components V2, or mobile scan order.
- If `Current request.params.sprint_phase == "initial"` and `Current request.params.initial_phase_step` is set, treat the request as a planner-only substep:
  ...
  - `artifact_sync`: update plan/spec/iteration-log artifacts only. Before user confirmation, this step must also return Codex-plan-mode style `proposals.initial_implementation_plan` with `PLAN-ACT-*` actions and then stop without backlog/TODO creation.
  - `backlog_definition`: derive sprint-relevant backlog from the current milestone, kickoff requirements, research prepass, `spec.md`, and the confirmed implementation plan; create or reopen backlog items before any selection. Backlog zero is invalid. Every backlog item must include concrete `acceptance_criteria` plus `origin.milestone_ref`, `origin.requirement_refs`, `origin.spec_refs`, and `origin.plan_action_refs`; it must also be linkable to the active sprint through `milestone_title` or `origin.sprint_id`. If source-backed research exists, every sprint-relevant backlog item must also include `origin.research_refs`.
    When `original_requirements` exist, `origin.requirement_refs` must cite `REQ-*` IDs and backlog coverage must include every `REQ-*`.
  ...
  - `todo_finalization`: finalize the execution-ready sprint backlog set and persist `planned_in_sprint_id` for chosen items. Verify every `REQ-*` has a requirement-slice todo, every non-supporting `PLAN-ACT-*` has at least one non-supporting TODO with matching `plan_action_refs`, and mark any supporting todo with `supporting_todo: true` plus the `REQ-*` it supports.
- If any initial-phase substep is reopened, treat `Current request.params.initial_phase_reopen_reason` as the blocking acceptance gap. Close the same substep only after the missing persisted evidence is present and read back.
