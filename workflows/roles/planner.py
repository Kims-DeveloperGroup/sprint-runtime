...
- If `Current request.result.proposals.research_report` or `research_prepass` exists, treat it as a planning input with leverage, not a footnote. Use its `milestone_refinement_hints`, `problem_framing_hints`, `spec_implications`, `todo_definition_hints`, `backing_reasoning`, `backing_sources`, and `todo_coverage_requirements` when refining milestones, writing specs, and defining backlog/todos.
- If a `report_artifact` exists, the research summary and `todo_coverage_requirements` are only an index. For every initial planning substep (`milestone_refinement`, `artifact_sync`, `backlog_definition`, `backlog_prioritization`, `todo_finalization`), read the full raw Markdown report artifact before completing the step and return `proposals.sprint_plan_update.research_report_read = {{report_artifact, raw_report_read: true, phase_step, referenced_sections, coverage_ids_considered}}`.
- In `backlog_definition` and `todo_finalization`, return `proposals.sprint_plan_update.research_coverage_matrix = [{{coverage_id, backlog_id, todo_id?, coverage_type}}]` and make sure every `RG-*` todo coverage requirement is covered by at least one sprint-relevant backlog item or selected sprint TODO.
...
- If `Current request.params.sprint_phase == "initial"` and `Current request.params.initial_phase_step` is set, treat the request as a planner-only substep:
  ...
  - `todo_finalization`: finalize the execution-ready sprint backlog set and persist `planned_in_sprint_id` for chosen items. Verify every `REQ-*` has a requirement-slice todo, every `RG-*` is covered by a selected TODO with `research_coverage_refs` plus matching `research_refs`, and mark any supporting todo with `supporting_todo: true` plus the `REQ-*` it supports. Unrelated milestone-relevant TODOs may omit research coverage refs.
