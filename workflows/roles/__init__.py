...
from teams_runtime.workflows.roles.planner import build_planner_role_rules
...
# Role capability metadata feeds orchestration routing without making role modules routing authorities.
...
class AgentUtilizationPolicy:
    ...
    implementation_reopen_limit: int
    load_error: str = ""
...
DEFAULT_AGENT_UTILIZATION_POLICY_DATA: dict[str, Any] = {
    ...
    "public_roles": {
        "orchestrator": {
            ...
            "allowed_next_roles": ["research", "planner", "designer", "architect", "developer", "qa"],
            "owned_intents": [],
        ...
        "research": {
            "summary": "Pre-planning research specialist for external grounding, evidence gathering, and source-backed planner guidance",
            "mission": "Decide whether external research is needed, gather sources when necessary, and hand planner a grounded research brief.",
            "strongest_for": [
                "source-backed research",
                "external grounding",
                "comparison and landscape scans",
                "evidence gathering",
            ],
            "preferred_skills": [
                "deep_research",
            ...
            "behavior_traits": ["evidence-driven", "source-backed", "planner-support"],
            "behavior_signals": ["evidence", "sources", "validate", "compare", "research"],
            "should_not_handle": ["repo-only implementation", "direct code edits", "commit execution"],
...
def build_agent_utilization_policy(
    ...
    *,
    ...
    return AgentUtilizationPolicy(
        ...
        implementation_reopen_limit=max(
            1,
            _coerce_int(workflow_contract.get("implementation_reopen_limit"), 3),
        ),
...
_AGENT_CAPABILITY_EXPORTS = [
    ...
    "AgentUtilizationPolicy",
    "RoutingWeights",
    "agent_utilization_policy_file",
    "build_agent_utilization_policy",
    "default_agent_utilization_policy",
    "get_agent_capability",
    "get_internal_agent_capabilities",
    "get_public_agent_capabilities",
    "intent_to_role_map",
...
RoleRulesBuilder = Callable[[str], str]
...
class RolePromptSpec:
    build_rules: RoleRulesBuilder
    extra_fields: str = ""


def _build_static_rules(builder: Callable[[], str]) -> RoleRulesBuilder:
    def _wrapped(_team_workspace_hint: str) -> str:
        return builder()
...
ROLE_PROMPT_SPECS: dict[str, RolePromptSpec] = {
    "orchestrator": RolePromptSpec(build_rules=build_orchestrator_role_rules),
    "planner": RolePromptSpec(build_rules=build_planner_role_rules),
    "designer": RolePromptSpec(build_rules=_build_static_rules(build_designer_role_rules)),
    "architect": RolePromptSpec(build_rules=_build_static_rules(build_architect_role_rules)),
    "developer": RolePromptSpec(build_rules=_build_static_rules(build_developer_role_rules)),
    "qa": RolePromptSpec(build_rules=_build_static_rules(build_qa_role_rules)),
    "version_controller": RolePromptSpec(
        build_rules=_build_static_rules(build_version_controller_role_rules),
        extra_fields=VERSION_CONTROLLER_EXTRA_FIELDS,
    ),
}


ROLE_MODULES = {
    "orchestrator": "teams_runtime.workflows.roles.orchestrator",
    "research": "teams_runtime.workflows.roles.research",
    "planner": "teams_runtime.workflows.roles.planner",
    "designer": "teams_runtime.workflows.roles.designer",
    "architect": "teams_runtime.workflows.roles.architect",
    "developer": "teams_runtime.workflows.roles.developer",
    "qa": "teams_runtime.workflows.roles.qa",
    "version_controller": "teams_runtime.workflows.roles.version_controller",
}


def get_role_prompt_spec(role: str) -> RolePromptSpec | None:
    return ROLE_PROMPT_SPECS.get(str(role or "").strip())


def render_role_prompt_spec(role: str, team_workspace_hint: str) -> tuple[str, str]:
    spec = get_role_prompt_spec(role)
    if spec is None:
        return "", ""
    return spec.build_rules(team_workspace_hint), spec.extra_fields
