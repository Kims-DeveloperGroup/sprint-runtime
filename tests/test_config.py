...
"""
    ...
    def test_scaffold_workspace_and_load_configs(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("refactoring 기회", architect_prompt)
            self.assertIn("재사용 가능한 helper/function/logic 추출 기회", architect_prompt)
            self.assertIn("module boundary/isolation/responsibility 일관성 이슈", architect_prompt)
            self.assertIn("review_cycle_count >= review_cycle_limit", architect_prompt)
            self.assertIn("optional refactor/reuse/module-structure 개선만으로 developer revision을 강제하지 않는다", architect_prompt)
            self.assertIn("advisory insight나 follow-up context", architect_prompt)
            ...
            self.assertIn("Current request.designer_context", planner_prompt)
            self.assertIn("designer의 `lead / summary / defer` 우선순위", planner_prompt)
            self.assertIn("Components V2", architect_prompt)
            self.assertIn("Designer Contract", architect_prompt)
            self.assertIn("silently redesign하거나 downgrade하지 말고", developer_prompt)
            self.assertIn("mention safety를 보존한다", developer_prompt)
            ...
            qa_prompt = (workspace_root / "qa" / "AGENTS.md").read_text(encoding="utf-8")
            self.assertIn("evidence matrix", qa_prompt)
            self.assertIn("Current request.result", qa_prompt)
            self.assertIn("source of truth 순서", qa_prompt)
...
"""
    ...
    def test_cmd_config_research_set_updates_runtime_yaml_without_restart(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("reasoning_level=Extended", rendered)
    ...
    def test_main_config_research_set_accepts_reasoning_level(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            runtime_config = load_team_runtime_config(tmpdir)
            self.assertEqual(runtime_config.research_defaults.reasoning_level, "Extended")
            self.assertIn("reasoning_level=Extended", output.getvalue())
