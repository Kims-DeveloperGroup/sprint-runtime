...
class TeamsRuntimeDesignerRoleTests(unittest.TestCase):
    def test_build_designer_role_rules_mentions_advisory_only_workflow_contract(self):
        ...
        self.assertIn("intent, audience, urgency, required action, and the scan path", rules)
        self.assertIn("`lead / summary / defer` hierarchy", rules)
        self.assertIn("Discord surfaces deliberately", rules)
        self.assertIn("mention policy and allowed-mentions expectations", rules)
        self.assertIn("`surface_contract`", rules)
        self.assertIn("`message_priority` concrete with at least `lead`, `summary`, and `defer`", rules)
    ...
    def test_role_runtime_prompt_uses_designer_role_rules(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("mention policy and allowed-mentions expectations", prompt)
            self.assertIn("`message_priority` concrete with at least `lead`, `summary`, and `defer`", prompt)
