...
class TeamsRuntimeQaRoleTests(unittest.TestCase):
    def test_build_qa_role_rules_mentions_validation_and_ux_reopen(self):
        ...
        self.assertIn("standard-library `unittest` runner", rules)
        ...
        self.assertIn("new or changed function/feature", rules)
        self.assertIn("final output that is a Discord message", rules)
        self.assertIn("./.agents/skills/discord_message_read/", rules)
        self.assertIn("channel_id` and `message_id", rules)
        self.assertIn("last_discord_message_ids", rules)
    ...
    def test_role_runtime_prompt_uses_qa_role_rules(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("standard-library `unittest` runner", prompt)
            ...
            self.assertIn("new or changed function/feature", prompt)
            self.assertIn("final output that is a Discord message", prompt)
            self.assertIn("./.agents/skills/discord_message_read/", prompt)
            self.assertIn("last_discord_message_ids", prompt)
