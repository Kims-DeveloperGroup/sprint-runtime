...
class RuntimePaths:
    ...
    def session_state_file(self, role: str, *, runtime_identity: str | None = None) -> Path:
        ...
        return self.role_sessions_dir / f"{identity}.json"
    ...
    def sprint_research_dir(self, sprint_id: str) -> Path:
        normalized = str(sprint_id or "").strip() or "unknown-sprint"
        return self.sprint_artifacts_root / normalized / "research"

    def sprint_research_file(self, sprint_id: str, request_id: str) -> Path:
        normalized_request_id = str(request_id or "").strip() or "unknown-request"
        return self.sprint_research_dir(sprint_id) / f"{normalized_request_id}.md"
